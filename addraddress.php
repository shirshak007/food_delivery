...
<title>Add Restaurant Address</title>
...
function func1()
{
		...
		if(document.f1.ph_no.value=="")
	{
	   alert("Mobile no field should not be empty");
	   document.f1.ph_no.focus();
	   return false
	}
	if(isNaN(document.f1.ph_no.value))
	{
	  alert("mobile no field should be numeric");
	document.f1.ph_no.value=""
	   document.f1.ph_no.focus();
	   return false
	}
	if(document.f1.ph_no.value.length!=10)
	{
	  alert("mobile no field should be 10 digit");
	   document.f1.ph_no.focus();
	   return false
	}
	if(document.f1.build.value=="")
	{
	   alert("Building No field should not be empty");
	   document.f1.build.focus();
	   return false
	}
...
<form action="addraddress2.php" method=post name=f1 onsubmit="return func1()">

	Address ID<font color=red>*</font>(Auto Generated)
	<input type=text name=r_loc_id readonly value="<?php
...
$sql = "SELECT * FROM r_loc ORDER BY R_LOC_ID DESC LIMIT 1";
